...
def replaceurl(text):
    ...
    #text = re.sub('((https?://[^\s]+)|(http?://[^\s]+)|(www\.[^\s]+))','url',text)
    #text = re.sub('http//url','url',text)
    #text = re.sub('https//url','url',text)
...
def countURL(text):
    return len(re.findall('((www\.[^\s]+)|(https?://[^\s]+))',text))
...
def counthashAt(text):
    return (len(re.findall(r'#[^\s]+',text))+len(re.findall('@[^\s]+',text)))
...
def countPunct(text):
    return (countMultiStopMarks(text)+countMultiExclamationMarks(text)+countMultiQuestionMarks(text))
